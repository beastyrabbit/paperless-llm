    ...
    const process = (input: DocumentLinksGraphInput) =>
      Effect.gen(function* () {
        ...
        return {
          success: true,
          value: null,
          reasoning: 'Compatibility document links agent does not infer links outside PiDocumentAgent.',
          confidence: 0,
          alternatives: [],
          attempts: 1,
          needsReview: false,
      ...
      }).pipe(Effect.mapError((error) => new AgentError({ message: `Document links compatibility agent failed: ${String(error)}`, agent: 'document_links', cause: error })));
