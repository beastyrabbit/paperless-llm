...
export const TitleAgentGraphService =
  Context.GenericTag<TitleAgentGraphService>("TitleAgentGraphService");
  ...
  Effect.gen(function* () {
    ...
    const process = (input: TitleInput) =>
      Effect.gen(function* () {
        ...
        yield* paperless
          .transitionDocumentTag(input.docId, tagConfig.summaryDone, tagConfig.titleDone)
        ...
        Effect.mapError(
          (error) =>
            new AgentError({
              message: `Title compatibility agent failed: ${String(error)}`,
              agent: "title",
              cause: error,
            }),
        ),
