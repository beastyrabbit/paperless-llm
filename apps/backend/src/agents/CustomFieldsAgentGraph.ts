    ...
    const process = (input: CustomFieldsGraphInput) =>
      Effect.gen(function* () {
        ...
        return {
          success: true,
          value: null,
          reasoning: 'Compatibility custom fields agent does not infer fields outside PiDocumentAgent.',
          confidence: 0,
          alternatives: [],
          attempts: 1,
          needsReview: false,
      ...
      }).pipe(Effect.mapError((error) => new AgentError({ message: `Custom fields compatibility agent failed: ${String(error)}`, agent: 'custom_fields', cause: error })));
